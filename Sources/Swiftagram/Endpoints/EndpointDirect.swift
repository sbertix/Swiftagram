...
public extension Endpoint {
    ...
    struct Direct {
        ...
        public static func threads(startingAt page: String? = nil) -> Paginated<ConversationCollection> {
            return base.inbox
                .appending(query: ["visual_message_return_type": "unseen",
                                   ...
                                   "limit": "20"])
                .paginating(process: ConversationCollection.self, key: "cursor", keyPath: \.oldestCursor, value: page)
                .locking(Secret.self)
        }

        /// All pending threads.
        /// - parameter page: An optional `String` holding reference to a valid cursor. Defaults to `nil`.
        public static func pendingThreads(startingAt page: String? = nil) -> Paginated<ConversationCollection> {
        ...
        public static func recipients(matching query: String? = nil) -> Disposable<RecipientCollection> {
            return base.appending(path: "ranked_recipients/")
                .appending(header: ["mode": "raven",
                                    "query": query ?? "",
                                    "show_threads": "true"])
                .prepare(process: RecipientCollection.self)
                .locking(Secret.self)
        }
        ...
        public static func thread(matching identifier: String,
                                  startingAt page: String? = nil) -> Paginated<ConversationUnit> {
            return base.threads
                .appending(path: identifier)
                .appending(query: ["visual_message_return_type": "unseen",
                                   "direction": "older",
                                   "limit": "20"])
                .paginating(process: ConversationUnit.self, key: "cursor", keyPath: \.thread.oldestCursor, value: page)
                .locking(Secret.self)
        }
