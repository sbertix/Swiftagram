...
public extension Endpoint {
    ...
    struct User {
        ...
        public static func summary(for identifier: String) -> Locked<ComposableRequest> {
            return base.append(identifier).info.locked()
        }

        /// All user matching `query`.
        /// - parameter query: A `String` holding reference to a valid user query.
        public static func all(matching query: String) -> Locked<Paginated<ComposableRequest>> {
            return base.search.query("q", value: query).paginating().locked()
        }
    }
}
